...
## Editor Library Options (Research Notes)

Three external editor libraries were evaluated as potential alternatives to the custom token-level engine described below. All have tradeoffs against CWOC's no-npm, no-build-step, vanilla JS architecture.

### Milkdown

- Plugin-driven WYSIWYG markdown editor built on ProseMirror + Remark
- True WYSIWYG: renders markdown inline as you type (bold looks bold, headers resize, etc.)
- Highly extensible via plugins (slash commands, tables, collaborative editing)
- Framework-agnostic with React, Vue, and vanilla JS integrations
- **Pros**: Closest to the Obsidian-style experience; markdown-native data model; headless/themeable
- **Cons**: npm ecosystem (`@milkdown/core` + plugins); no official single-file CDN bundle; would require a custom pre-bundle or finding a community UMD build to work without a build step

### Tiptap

- Built on ProseMirror; rich text WYSIWYG editor with extension system
- Very extensible (task lists, tables, mentions, drag handles, etc.)
- Large community and good documentation
- **Pros**: Polished UX; strong extension ecosystem; collaborative editing support
- **Cons**: npm-only (`@tiptap/core` + extensions); no official CDN bundle; oriented toward rich text (HTML) rather than markdown — markdown support is an add-on layer, not the native format; would need self-bundling to use without npm

### CodeMirror 6

- Code/text editor (not WYSIWYG) — shows raw markdown with syntax highlighting
- Ground-up rewrite of CodeMirror; extremely mature and well-architected
- Supports extensions (vim keybindings, search/replace, bracket matching, autocomplete)
- **Pros**: CDN-possible (community UMD builds exist via jsDelivr/ESM.sh); lightweight and fast; excellent mobile support; accessible; markdown syntax highlighting makes raw editing much nicer
- **Cons**: Not WYSIWYG — users see `## Heading` not a rendered heading; power-user oriented; CDN setup requires some effort to assemble the right bundle

### Summary Table

| | CDN-ready? | WYSIWYG? | Markdown-native? | Mobile? |
|---|---|---|---|---|
| **Milkdown** | ❌ Not without custom bundle | ✅ Yes | ✅ Yes | ✅ |
| **Tiptap** | ❌ Not without custom bundle | ✅ Yes | ⚠️ Add-on | ✅ |
| **CodeMirror 6** | ⚠️ Possible with effort | ❌ No (syntax-highlighted source) | ✅ Yes | ✅ |
| **Custom engine (below)** | ✅ Already vanilla JS | ✅ Hybrid (token-level) | ✅ Yes | ✅ |

### Conclusion

None of the three libraries are drop-in compatible with CWOC's no-build architecture without significant bundling work. CodeMirror 6 is the most realistic external option if a CDN build can be assembled, but it only provides syntax-highlighted source editing, not the Obsidian-style live preview behavior. The custom token-level engine described in this spec remains the best fit for achieving true inline live preview without introducing a build step.
