...
- [ ] 1. Create PWA static files and manifest
  - [ ] 1.1 Create `src/pwa/manifest.json` with all required fields
    - name, short_name, display, start_url, scope, theme_color, background_color, orientation, icons array (192 + 512)
    - _Requirements: 1.1–1.11_
  - [ ] 1.2 Create placeholder icon files `src/pwa/cwoc-icon-192.png` and `src/pwa/cwoc-icon-512.png`
    - Scale from existing `src/static/cwod_logo.png` or create placeholder PNGs at correct dimensions
    - _Requirements: 3.1, 3.2, 3.3_
  - [ ] 1.3 Create `src/pwa/offline.html` offline fallback page
    - CWOC parchment theme, friendly "no network" message, retry suggestion
    - Use `shared-page.css` for consistent styling
    - _Requirements: 6.1, 6.3, 6.4_

- [ ] 2. Create service worker
  - [ ] 2.1 Create `src/pwa/sw.js` with install, activate, fetch, push, and notificationclick handlers
...
- [ ] 3. Create PWA registration script
  - [ ] 3.1 Create `src/pwa/pwa-register.js`
  ...
  - [ ] 3.2 Add push subscription logic to `src/pwa/pwa-register.js`
...
- [ ] 4. Checkpoint — Review PWA frontend files
  - Ensure all files in `src/pwa/` are complete and consistent. Ask the user if questions arise.

- [ ] 5. Add PWA meta tags to all HTML pages
  - [ ] 5.1 Add PWA `<head>` tags to all 14 HTML pages (excluding `_template.html`)
  ...
  - [ ] 5.2 Update `src/frontend/html/_template.html` with PWA tags as reference for future pages
    - Add the same PWA head tags and pwa-register.js script tag
    - _Requirements: 2.1–2.5_

- [ ] 6. Add install button to dashboard sidebar
  - [ ] 6.1 Add install button HTML to `src/frontend/html/index.html` sidebar
    - Hidden by default, shown when `beforeinstallprompt` fires
    - _Requirements: 8.2_
  - [ ] 6.2 Add install button click handler in `src/frontend/js/dashboard/main-init.js`
    - Wire button to `handleInstallClick()` from pwa-register.js
    - _Requirements: 8.3, 8.4, 8.5_

- [ ] 7. Backend: serve SW and manifest, add push migrations
  - [ ] 7.1 Add routes in `src/backend/main.py` to serve `/sw.js` and `/manifest.json` from `src/pwa/`
  ...
  - [ ] 7.2 Add migration functions in `src/backend/migrations.py`
  ...
  - [ ] 7.3 Register new migrations in `src/backend/main.py` startup sequence
    - Import and call `migrate_add_push_subscriptions()` and `migrate_add_vapid_keys()` after existing migrations
    - _Requirements: 11.1, 11.5_

- [ ] 8. Backend: push notification routes
  - [ ] 8.1 Create `src/backend/routes/push.py` with push API endpoints
  ...
  - [ ] 8.2 Register push router in `src/backend/main.py`
    - Import and include `push_router` from `src.backend.routes.push`
    - _Requirements: 11.2_

- [ ] 9. Checkpoint — Backend push routes complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Backend: wire push into alert scheduler
  - [ ] 10.1 Add push notification trigger to `src/backend/weather.py` alert scheduler
...
- [ ] 11. Update configurator script (CONFIRM WITH USER BEFORE MODIFYING)
  - [ ] 11.1 Update `install_python_deps()` in `install/configurinator.sh` to include `pywebpush` in the `required_pkgs` list
...
- [ ] 12. Write backend tests for push routes
  - [ ]* 12.1 Write unit tests in `src/backend/test_push.py`
...
- [ ] 14. Update index, version, and release notes
  - [ ] 14.1 Update `src/INDEX.md` with all new files, functions, and routes
    - _Requirements: 12.1–12.7_
  - [ ] 14.2 Update `src/VERSION` with current timestamp
    - Run `date "+%Y%m%d.%H%M"` and use the returned value
  - [ ] 14.3 Create release notes file in `documents/release_notes/`
