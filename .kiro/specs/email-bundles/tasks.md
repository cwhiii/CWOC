...
- [x] 1. Database migration and Pydantic models
  - [x] 1.1 Add `migrate_create_bundles_tables()` to `src/backend/migrations.py`
  ...
  - [x] 1.2 Register migration call in `src/backend/main.py`
    - Import and call `migrate_create_bundles_tables()` alongside existing migration calls at startup
    - _Requirements: 1.2_

  - [x] 1.3 Add Pydantic models to `src/backend/models.py`
...
- [x] 2. Bundle CRUD API routes
  - [x] 2.1 Create `src/backend/routes/bundles.py` with GET `/api/bundles` endpoint
  ...
  - [x] 2.2 Add POST `/api/bundles` endpoint for bundle creation
  ...
  - [x] 2.3 Add PUT `/api/bundles/{bundle_id}` endpoint for bundle update
  ...
  - [x] 2.4 Add DELETE `/api/bundles/{bundle_id}` endpoint for bundle deletion
  ...
  - [x] 2.5 Add PUT `/api/bundles/reorder` endpoint
  ...
  - [x] 2.6 Add POST `/api/bundles/{bundle_id}/rules` and DELETE `/api/bundles/{bundle_id}/rules/{rule_id}` endpoints
  ...
  - [x] 2.7 Implement `_initialize_default_bundles(owner_id)` helper
  ...
  - [x] 2.8 Register bundles router in `src/backend/main.py`
    - Import `bundles_router` from `src.backend.routes.bundles`
    - Call `app.include_router(bundles_router)`
    - _Requirements: 8.1_

- [x] 3. Bundle classification engine
  - [x] 3.1 Implement `classify_email_into_bundle()` (single-placement) in `src/backend/routes/bundles.py`
  ...
  - [x] 3.2 Implement `classify_email_into_bundles()` (multi-placement) in `src/backend/routes/bundles.py`
    - Load bundles sorted by display_order ASC
  ...
  - [x] 3.3 Integrate classification into email sync flow in `src/backend/routes/email.py`
...
- [x] 5. Frontend — Bundle toolbar and tab rendering
  - [x] 5.1 Create `src/frontend/css/dashboard/styles-email-bundles.css`
  ...
  - [x] 5.2 Create `src/frontend/js/dashboard/main-email-bundles.js`
  ...
  - [x] 5.3 Integrate bundle toolbar into `src/frontend/js/dashboard/main-email.js`
...
- [x] 6. Frontend — Bundle creation modal
  - [x] 6.1 Implement bundle modal in `src/frontend/js/dashboard/main-email-bundles.js`
...
- [x] 7. Frontend — Context menu for bundle management
  - [x] 7.1 Implement bundle context menu in `src/frontend/js/dashboard/main-email-bundles.js`
  ...
  - [x] 7.2 Add bundle modal and context menu CSS to `styles-email-bundles.css`
...
- [x] 8. Settings page — Multi-placement toggle
  - [x] 8.1 Add multi-placement toggle to settings page
  ...
  - [x] 8.2 Ensure `bundles_multi_placement` is included in settings API responses
...
- [x] 10. Property-based tests
  - [x] 10.1 Write property test for Bundle CRUD Round-Trip
  ...
  - [x] 10.2 Write property test for Owner Scoping Isolation
  ...
  - [x] 10.3 Write property test for Bundle Rename Tag Migration
  ...
  - [x] 10.4 Write property test for Bundle Delete Tag Cleanup
  ...
  - [x] 10.5 Write property test for Bundle List Sort Order
  ...
  - [x] 10.6 Write property test for Bundle Filtering Correctness
  ...
  - [x] 10.7 Write property test for Unread Count Computation
  ...
  - [x] 10.8 Write property test for Bundle Name Validation
  ...
  - [x] 10.9 Write property test for Bundle Reorder Persistence
  ...
  - [x] 10.10 Write property test for Bundle Filter Composes with Sub-Filter
  ...
  - [x] 10.11 Write property test for Single-Placement Priority Ordering
  ...
  - [x] 10.12 Write property test for Multi-Placement Completeness
...
- [x] 11. Unit tests for bundle API and classification
  - [x] 11.1 Write unit tests in `src/backend/test_email_bundles.py`
...
- [x] 12. Final wiring and version update
  - [x] 12.1 Update `src/INDEX.md` with all new functions, routes, files, and CSS sections
  ...
  - [x] 12.2 Update `src/VERSION` with current timestamp
    - Run `date "+%Y%m%d.%H%M"` and use the returned value
    - Only do this once at the very end

  - [x] 12.3 Create release notes file
