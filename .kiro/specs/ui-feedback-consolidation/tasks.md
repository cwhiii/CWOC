...
- [ ] 1. Add `.cwoc-overlay` CSS class to shared-page.css
...
- [ ] 2. Implement `cwocUndoToast()` in shared-utils.js
  - [ ] 2.1 Add the `cwocUndoToast(message, opts)` function after the existing `cwocToast` function
  ...
  - [ ] 2.2 Refactor `_showDeleteUndoToast` in shared.js to delegate to `cwocUndoToast`
...
- [ ] 3. Delete email undo toast duplicates
  - [ ] 3.1 Delete `_emailUndoToast` function from `main-email.js`
  ...
  - [ ] 3.2 Delete `_emailShowUndoSendBar` function from `main-email.js`
  ...
  - [ ] 3.3 Delete duplicate email undo CSS
...
- [ ] 4. Replace auto-fade modals with cwocToast
  - [ ] 4.1 Replace settings.html auto-fade modals
  ...
  - [ ] 4.2 Replace shared-tag-modal.js auto-fade modals
...
- [ ] 5. Implement `cwocUnsavedModal()` in shared-utils.js
  - [ ] 5.1 Add the `cwocUnsavedModal(opts)` function
  ...
  - [ ] 5.2 Replace unsaved-modal in shared-page.js (CwocSaveSystem)
  ...
  - [ ] 5.3 Replace unsaved-modal in editor-save.js
    - Find `_navigateWithSaveCheck` inline modal construction
    - Replace with `cwocUnsavedModal(opts).then(result => { ... })`
    - _Requirements: 4.8_

  - [ ] 5.4 Replace unsaved-modal in editor-send-item.js
    - Find the inline modal construction for navigating to new chit
    - Replace with `cwocUnsavedModal(opts).then(result => { ... })`
    - _Requirements: 4.9_

  - [ ] 5.5 Replace unsaved-modal in editor-prerequisites.js
    - Find the inline modal construction
    - Replace with `cwocUnsavedModal(opts).then(result => { ... })`
    - _Requirements: 4.10_

- [ ] 6. Migrate legacy HTML delete modals to cwocConfirm
  - [ ] 6.1 Migrate `#deleteChitModal` in editor
  ...
  - [ ] 6.2 Migrate `#deleteEmailAccountModal` in settings
  ...
  - [ ] 6.3 Migrate `#delete-modal` in settings (color deletion)
...
- [ ] 7. Apply `.cwoc-overlay` class to existing modal functions
  - [ ] 7.1 Update `cwocConfirm` to use `.cwoc-overlay` class
  ...
  - [ ] 7.2 Update `cwocPromptModal` to use `.cwoc-overlay` class
...
- [ ] 8. Verify and clean up
  - [ ] 8.1 Search for any remaining references to deleted functions/elements
  ...
  - [ ] 8.2 Verify ESC chain still works on all pages
...
- [ ] 9. Update INDEX.md, release notes, and version
