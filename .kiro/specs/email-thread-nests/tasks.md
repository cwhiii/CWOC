l# Implementation Plan: Email Thread Nests
...
- [x] 1. Add nest_thread_id column and model update
  ...
  - [x] 1.3 Write property test for nest reference validation (Property 1)
  ...
  - [x] 1.4 Write property test for subject label truncation (Property 3)
...
- [x] 2. Implement nest validation and cascade cleanup in chit CRUD
  - [x] 2.1 Add nest_thread_id validation to PUT/POST in `src/backend/routes/chits.py`
  ...
  - [x] 2.2 Add cascade cleanup on email chit permanent deletion in `src/backend/routes/trash.py`
    - When an email chit is permanently deleted, execute: `UPDATE chits SET nest_thread_id = NULL WHERE nest_thread_id = ?`
    - _Requirements: 1.5_

  - [x] 2.3 Write property test for cascade cleanup on delete (Property 2)
  ...
  - [x] 2.4 Write property test for API round-trip (Property 10)
...
- [x] 4. Implement recent threads endpoint and extend thread endpoint
  - [x] 4.1 Add `GET /api/email/threads/recent` endpoint in `src/backend/routes/email.py`
  ...
  - [x] 4.2 Extend `GET /api/email/thread/{chit_id}` to include nested chits
  ...
  - [x] 4.3 Write property test for thread search filtering (Property 5)
  ...
  - [x] 4.4 Write property test for thread endpoint includes nests (Property 11)
...
- [x] 5. Implement nest button and thread picker frontend
  - [x] 5.1 Create `src/frontend/js/editor/editor-nest.js` with nest button logic
  ...
  - [x] 5.2 Add nest button HTML and thread picker template to `src/frontend/html/editor.html`
  ...
  - [x] 5.3 Integrate nest button with editor save flow in `src/frontend/js/editor/editor-save.js`
    - Include `getNestData()` result in the save payload
    - Ensure nest_thread_id is sent on PUT /api/chits/{id}
    - _Requirements: 4.3, 7.2_

  - [x] 5.4 Write property test for nest button visibility (Property 4)
...
- [x] 7. Implement nest injection in email thread view
  - [x] 7.1 Add nest injection logic to `src/frontend/js/dashboard/main-email.js`
  ...
  - [x] 7.2 Implement `_buildNestedChitCard(chit)` in `src/frontend/js/dashboard/main-email.js`
  ...
  - [x] 7.3 Modify `_buildThreadedEmailCard()` and `_toggleThreadExpand()` in `main-email.js`
  ...
  - [x] 7.4 Write property test for nested chit thread membership (Property 6)
  ...
  - [x] 7.5 Write property test for nested chit sort order (Property 7)
  ...
  - [x] 7.6 Write property test for top card invariant (Property 8)
  ...
  - [x] 7.7 Write property test for inbox exclusion invariant (Property 9)
...
- [x] 8. Implement nest injection in editor thread display
  - [x] 8.1 Update editor email zone thread display to render nested chits
...
- [x] 9. Add CSS for nest button, picker modal, and nested chit cards
  - [x] 9.1 Create `src/frontend/css/editor/editor-nest.css`
  ...
  - [x] 9.2 Link `editor-nest.css` in `src/frontend/html/editor.html`
...
- [x] 11. Update INDEX.md, VERSION, and release notes
  - [x] 11.1 Update `src/INDEX.md` with all new functions, endpoints, and files
  ...
  - [x] 11.2 Update `src/VERSION` with current timestamp
    - Run `date "+%Y%m%d.%H%M"` and use the returned value

  - [x] 11.3 Create release notes file
