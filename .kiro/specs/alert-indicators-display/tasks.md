...
- [x] 1. Implement alert detection and indicator helpers in `frontend/shared.js`
  - [x] 1.1 Add `_chitHasAlerts(chit)` function to `frontend/shared.js`
  ...
  - [x] 1.2 Add `_getAlertIndicators(chit, settings, context)` function to `frontend/shared.js`
  ...
  - [x] 1.3 Write property test: Alert detection with legacy flags (Property 3)
  ...
  - [x] 1.4 Write property test: Backward-compatible defaults (Property 7)
  ...
  - [x] 1.5 Write property test: Calendar display mode correctness (Property 1)
  ...
  - [x] 1.6 Write property test: Calendar icon uniqueness (Property 2)
  ...
  - [x] 1.7 Write property test: Combined mode single icon on cards (Property 4)
  ...
  - [x] 1.8 Write property test: Individual mode per-type icons on cards (Property 5)
  ...
  - [x] 1.9 Write property test: "If Space" resolves to show for non-month contexts (Property 6)
...
- [x] 2. Checkpoint — Verify helpers
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Wire alert indicators into calendar views via `calendarEventTitle` in `frontend/shared.js`
  - [x] 3.1 Modify `calendarEventTitle(chit, isDueOnly, info)` to accept a `settings` parameter and a `context` parameter
  ...
  - [x] 3.2 Update all calendar view renderers in `frontend/main.js` to pass cached settings and context to `calendarEventTitle`
...
- [x] 4. Wire alert indicators into card-style views via `_buildChitHeader` in `frontend/main.js`
  - [x] 4.1 Modify `_buildChitHeader(chit, titleHtml)` to accept a `settings` parameter
  ...
  - [x] 4.2 Update `displayChecklists()`, `displayTasks()`, `displayNotes()`, and `displayAlarms()` in `frontend/main.js` to pass cached `visual_indicators` settings to `_buildChitHeader`
    - Retrieve settings from `getCachedSettings()` and pass `visual_indicators` object
    - Do NOT modify `displayProjects()` — no alert indicators on Projects view
    - _Requirements: 2.2, 2.3_

- [x] 5. Checkpoint — Verify dashboard rendering
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Add Combine Alerts toggle and new dropdowns to Settings page
  - [x] 6.1 Add HTML elements to `frontend/settings.html` in the Visual Indicators section
  ...
  - [x] 6.2 Add `_toggleCombineAlerts()` function to `frontend/settings.js`
  ...
  - [x] 6.3 Update `SettingsManager.updateForm()` in `frontend/settings.js` to load new settings
  ...
  - [x] 6.4 Update `SettingsManager.gatherSettings()` in `frontend/settings.js` to include new keys
...
- [x] 7. Checkpoint — Verify settings round-trip
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Update help documentation and version
  - [x] 8.1 Update `frontend/help.html` with documentation for the new alert indicators feature
  ...
  - [x] 8.2 Update the `VERSION` file at project root with the current date/time stamp
