...
- [x] 10. Phase 10 — Move static assets to src/static/
  - [x] 10.1 Move all static assets to `src/static/`
    ...
    - _Requirements: 1.1_
  - [x] 10.2 Update `StaticFiles` mount for static assets
    - Update the `/static/` mount in `src/backend/main.py` to serve from `src/static/`
    - _Requirements: 1.4, 9.6_
  - [x] 10.3 Verify all static assets load correctly
  ...
  - [x] 10.4 Remove old `static/` directory
    - After all assets are moved and verified, remove the now-empty `static/` directory
    - _Requirements: 1.1_
  - [x] 10.5 Update `mega_restructure_plan.md` — mark Phase 10 complete
    - _Requirements: 14.6_
