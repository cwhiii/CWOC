...
- [ ] 1. Database migrations — add email columns to chits table and email_account to settings
  - [ ] 1.1 Add `migrate_add_email_fields()` function to `src/backend/migrations.py`
...
- [ ] 2. Backend models — add email fields to Chit and email_account to Settings
  - [ ] 2.1 Add email fields to the `Chit` Pydantic model in `src/backend/models.py`
  ...
  - [ ] 2.2 Add `email_account` field to the `Settings` Pydantic model in `src/backend/models.py`
    - `email_account: Optional[str] = None` — JSON string containing email config
    - _Requirements: 1.2_
  - [ ] 2.3 Update chit serialization/deserialization in `src/backend/routes/chits.py`
  ...
  - [ ] 2.4 Update settings serialization in `src/backend/routes/settings.py`
...
- [ ] 3. Reserved tag namespace enforcement — protect `CWOC_System/` prefix
  - [ ] 3.1 Add backend validation helpers
  ...
  - [ ] 3.2 Enforce in chit create/update routes
  ...
  - [ ] 3.3 Enforce in settings save route
  ...
  - [ ] 3.4 Add frontend validation for tag creation
...
- [ ] 5. Crypto module — Fernet encryption with graceful fallback
  - [ ] 5.1 Create crypto helper functions in `src/backend/routes/email.py`
...
- [ ] 6. System tag integration — add `CWOC_System/Email` to `compute_system_tags`
  - [ ] 6.1 Update `compute_system_tags` in `src/backend/db.py`
...
- [ ] 7. Email sync engine — IMAP fetch, parse, store
  - [ ] 7.1 Implement IMAP connection and sync functions in `src/backend/routes/email.py`
  ...
  - [ ] 7.2 Implement email parsing functions
  ...
  - [ ] 7.3 Implement chit creation from parsed email
  ...
  - [ ] 7.4 Implement backfill estimation function
...
- [ ] 8. Email send module — SMTP compose, send
  - [ ] 8.1 Implement SMTP connection and send functions in `src/backend/routes/email.py`
  ...
  - [ ] 8.2 Implement reply and forward helpers
...
- [ ] 10. Backend email routes — FastAPI router with all endpoints
  - [ ] 10.1 Create `src/backend/routes/email.py` router scaffold and register it
  ...
  - [ ] 10.2 Implement `POST /api/email/sync` endpoint
  ...
  - [ ] 10.3 Implement `POST /api/email/send/{chit_id}` endpoint
  ...
  - [ ] 10.4 Implement `PATCH /api/email/{chit_id}/read` endpoint
    - Set `email_read` to true on the specified email chit
    - _Requirements: 10.4, 8.1_
  - [ ] 10.5 Implement `POST /api/email/test-connection` endpoint
  ...
  - [ ] 10.6 Implement `POST /api/email/backfill-estimate` endpoint
...
- [ ] 11. Email zone in chit editor — HTML + JS + CSS
  - [ ] 11.1 Add Email zone HTML to `src/frontend/html/editor.html`
  ...
  - [ ] 11.2 Create `src/frontend/js/editor/editor-email.js`
  ...
  - [ ] 11.3 Create `src/frontend/css/editor/editor-email.css`
  ...
  - [ ] 11.4 Integrate email zone into editor save/load flow
...
- [ ] 12. Email dashboard tab — main-email.js + tab bar integration
  - [ ] 12.1 Add Email tab to dashboard tab bar in `src/frontend/html/index.html`
    - Add "Email" tab button with icon + label, following existing tab pattern
    - Add keyboard shortcut for the Email tab
    - _Requirements: 5.1_
  - [ ] 12.2 Create `src/frontend/js/dashboard/main-email.js`
  ...
  - [ ] 12.3 Integrate email tab into dashboard dispatch
...
- [ ] 13. Email deletion and restore — trash integration
  - [ ] 13.1 Implement email delete behavior
...
- [ ] 15. Settings page email account section
  - [ ] 15.1 Add "Email Account" section to `src/frontend/html/settings.html`
  ...
  - [ ] 15.2 Add settings page JavaScript for email account
...
- [ ] 16. Deployment script updates — configurinator.sh + cwoc-push.sh
  - [ ] 16.1 Update `install/configurinator.sh` to install `cryptography` package
    ...
    - _Requirements: 1.5_
  - [ ] 16.2 Update `cwoc-push.sh` to install `cryptography` on push
...
- [ ] 18. Update INDEX.md, version, and release notes
  - [ ] 18.1 Update `src/INDEX.md` with all new files and functions
    ...
    - _Requirements: all_
  - [ ] 18.2 Update `src/VERSION` with current timestamp
    - Run `date "+%Y%m%d.%H%M"` and write the result to `src/VERSION`
    - _Requirements: all_
  - [ ] 18.3 Create release notes
