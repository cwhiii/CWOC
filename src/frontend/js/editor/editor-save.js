...
async function buildChitObject() {
  ...
  var pitTimeVal = pitTimeBtn ? (pitTimeBtn.dataset.time || '') : '';
...
/**
 * Handle exit when auto-save is enabled.
 * - If all saved: navigate immediately
 * - If save in progress: wait for completion, then navigate
 * - If changes pending: trigger immediate save, then navigate on success
 * - If immediate save fails: show Discard/Retry modal
 */
async function _handleAutoSaveExit() {
  var autoSave = window._autoSave;
  var state = autoSave.getState();
  var url = _getEditorReturnUrl();

  // All saved — navigate immediately without prompt
  if (state === 'saved') {
    _cancelServerTimersForChit();
  ...
  // Save in progress — wait for it to complete, then navigate
  if (state === 'saving') {
    // Poll until save completes (check every 200ms, timeout after 10s)
    var waited = 0;
    var pollInterval = setInterval(function () {
      waited += 200;
      var currentState = autoSave.getState();
      if (currentState === 'saved') {
        clearInterval(pollInterval);
        _cancelServerTimersForChit();
        window._cwocSkipBeforeUnload = true;
        window.location.href = url;
      } else if (currentState === 'error' || waited >= 10000) {
        clearInterval(pollInterval);
        // Save failed or timed out — show Discard/Retry modal
        _showAutoSaveExitFailModal(url);
      }
    }, 200);
    return;
  }

  // Changes pending (or error state) — trigger immediate save
  try {
    ...
    console.error('[cancelOrExit] Auto-save immediate save failed:', e);
    _showAutoSaveExitFailModal(url);
  }
}

/**
 * Show a Discard/Retry modal when auto-save exit fails.
 * Uses cwocConfirm pattern for consistent parchment-themed modals.
 */
async function _showAutoSaveExitFailModal(url) {
  var existing = document.getElementById('cwoc-autosave-exit-modal');
  if (existing) existing.remove();

  var overlay = document.createElement('div');
  overlay.id = 'cwoc-autosave-exit-modal';
  overlay.className = 'modal';
  overlay.style.display = 'flex';
  overlay.innerHTML = '<div class="modal-content">'
    + '<h3>Save Failed</h3>'
    + '<p>Your changes could not be saved. What would you like to do?</p>'
    + '<div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;">'
    + '<button class="standard-button" id="cwoc-autosave-retry">🔄 Retry</button>'
    + '<button class="standard-button" id="cwoc-autosave-discard" style="background:#a0522d;color:#fdf5e6;">🗑️ Discard</button>'
    + '</div>'
    + '</div>';
  document.body.appendChild(overlay);

  document.getElementById('cwoc-autosave-retry').onclick = function () {
    overlay.remove();
    // Retry the exit flow
    _handleAutoSaveExit();
  };

  document.getElementById('cwoc-autosave-discard').onclick = function () {
    overlay.remove();
  ...
  // ESC closes the modal (treated as cancel — stay on page)
...
/**
 * Navigate to a URL with an unsaved-changes check.
 * If there are unsaved changes, shows the CWOC modal with Save/Discard/Cancel.
