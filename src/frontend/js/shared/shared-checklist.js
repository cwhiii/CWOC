...
/**
 * Render markdown for a checklist item's text element.
 * Uses marked.js full parse to support block-level elements (headers, lists,
 * numbered lists, blockquotes) in multi-line checklist items, as well as
 * inline formatting (bold, italic, links, code, strikethrough).
 * Falls back to plain textContent if marked.js is not available.
 * @param {HTMLElement} el - The element to render into
 * @param {string} text - The raw markdown text
 */
function renderChecklistItemMarkdown(el, text) {
  if (!text) { el.textContent = ''; return; }
  if (typeof marked === 'undefined') { el.textContent = text; return; }
  if (typeof marked.parse === 'function') {
    var html = marked.parse(text, { breaks: true });
    // For single-line items, strip the outer <p> wrapper to keep it compact
    if (!text.includes('\n')) {
      html = html.replace(/^<p>(.*)<\/p>\s*$/s, '$1');
    }
    // Remove any <input> checkboxes generated by GFM task list syntax
    html = html.replace(/<input[^>]*>/gi, '');
    el.innerHTML = html;
    // Prevent rendered links from participating in tab order / focus
