...
export class LocalReflowEngine {
    ...
    async _requestBackendRender(spreadStartPage, spreadImages) {
        ...
        console.log('[LocalReflow] _requestBackendRender — payload:', JSON.stringify(payload));
        ...
        try {
            const result = await api.post(
                `/projects/${this.projectId}/illustrations/render-preview`,
                payload
            );

            console.log('[LocalReflow] _requestBackendRender — backend render received successfully');
            ...
            if (err.name === 'AbortError') {
                ...
                console.error('[LocalReflow] _requestBackendRender — backend render failed:', err.message, err);
