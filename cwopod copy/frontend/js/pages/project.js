...
const STEPS = [
    { id: 'source', label: 'Source', description: 'Book imported' },
...
export async function render($container, params) {
    ...
    function statusToStep(status) {
        if (status === 'print_ready' || status === 'ordered' || status === 'shipped') return 8;
        if (status === 'cover_ready') return 5;
        if (status === 'typeset') return 4;
        return 0;
    }
    ...
    function renderWizard() {
        ...
        $container.find('.step').on('click', async function () {
            ...
            if (step <= currentStep) {
                ...
                // Typeset gate: if navigating to typeset step (index 4), evaluate gate
                if (step === 4) {
    ...
    function renderStepContent() {
        ...
        switch (currentStep) {
            case 0: renderSourceStep($content); break;
            case 1: renderPrintSizeStep($content); break;
            case 2: renderTypoStep($content); break;
            case 3: renderIllustrationsStep($content); break;
            case 4: renderTypesetStep($content); break;
            case 5: renderCoverStep($content, 'prompts'); break;
            case 6: renderCoverStep($content, 'images'); break;
            case 7: renderCoverStep($content, 'builder'); break;
            case 8: renderPrintStep($content); break;
        }
    }
    ...
    function renderSourceStep($el) {
        ...
        $el.find('#continue-typo').on('click', () => { currentStep = 1; renderWizard(); });
    }
    ...
    function renderPrintSizeStep($el) {
        ...
        $el.html(`
            ...
            <button class="btn primary" id="save-trim-continue">Save & Continue to Typo Check →</button>
        `);
    ...
    function renderTypoStep($el) {
        $el.html(`
            ...
            <div class="actions">
                <button class="btn secondary" id="skip-typo">Skip to Illustrations →</button>
                <button class="btn primary" id="scan-typos">Scan for Typos</button>
            </div>
            <div id="corrections-area"></div>
        `);
        $el.find('#skip-typo').on('click', () => { currentStep = 3; renderWizard(); });
            ...
            try {
                ...
                if (data.status === 'complete') {
                    ...
                    if (corrections.length > 0) {
                        ...
                        $area.html('<p>No typos found! Your text looks clean.</p><button class="btn primary" id="advance-typeset">Continue to Illustrations →</button>');
                        $area.find('#advance-typeset').on('click', () => { currentStep = 3; renderWizard(); });
                    }
    ...
    function renderCorrections($area, corrections) {
        ...
        html += '</ul><button class="btn primary" id="apply-corrections">Apply & Continue to Illustrations →</button>';
        $area.html(html);
    ...
    async function renderIllustrationsStep($el) {
        ...
        $el.html(`
            ...
            <div class="actions" style="margin-top:1rem;">
                <button class="btn secondary" id="skip-illustrations">Skip to Typeset →</button>
            </div>
        `);
    ...
    function renderTypesetStep($el) {
        ...
        // Dedication page editor section
        html += `
            <div class="dedication-editor" style="margin-bottom:1.5rem; padding:1rem; border:1px solid var(--tan); border-radius:8px; background:var(--cream);">
                <h3 style="margin-bottom:0.5rem;">📖 Dedication Page <span style="font-weight:normal; font-size:0.85rem; color:var(--medium-brown);">(optional)</span></h3>
                <p style="font-size:0.9rem; color:var(--medium-brown); margin-bottom:0.75rem;">Add a personal message — perfect for gifts. Appears after the title page.</p>

                <div class="dedication-mode-tabs" style="display:flex; gap:0.5rem; margin-bottom:0.75rem;">
                    <button class="btn btn-sm" id="ded-mode-none" data-mode="none" style="font-size:0.8rem;">None</button>
                    <button class="btn btn-sm" id="ded-mode-text" data-mode="text" style="font-size:0.8rem;">✍️ Text</button>
                    <button class="btn btn-sm" id="ded-mode-image" data-mode="image" style="font-size:0.8rem;">🖼️ Image</button>
                </div>

                <div id="dedication-content"></div>
                <div id="dedication-status" style="margin-top:0.5rem;"></div>
            </div>
        `;

        if (project.interior_pdf_path) {
            html += `<p style="color:#008080; font-weight:600;">✓ Interior PDF already generated.</p><div class="actions"><a href="/api/projects/${projectId}/interior-pdf" class="btn secondary" download>Download PDF</a><button class="btn secondary" id="regenerate-typeset">Re-generate PDF</button><a href="/projects/${projectId}/cover" class="btn primary">Continue to Cover →</a></div><div id="typeset-status"></div>`;
        } else {
            html += `<p>Generate a professionally formatted interior PDF ready for print.</p><button class="btn primary" id="start-typeset">Generate Interior PDF</button><div id="typeset-status"></div>`;
        }
        ...
        // Initialize dedication editor
        initDedicationEditor($el);
    }

    async function initDedicationEditor($el) {
        console.log('[Dedication] Initializing dedication editor');
        const $content = $el.find('#dedication-content');
        const $status = $el.find('#dedication-status');
        let currentMode = 'none';

        // Load existing dedication data
        try {
            const data = await api.get(`/projects/${projectId}/dedication`);
            console.log('[Dedication] Loaded existing dedication:', data);
            currentMode = data.mode || 'none';
        } catch (err) {
            console.warn('[Dedication] Failed to load dedication:', err.message);
        }

        // Tab click handlers
        $el.find('.dedication-mode-tabs button').on('click', function () {
            const mode = $(this).data('mode');
            console.log('[Dedication] Mode switched to:', mode);
            currentMode = mode;
            updateModeTabs();
            renderDedicationContent();
        });

        function updateModeTabs() {
            $el.find('.dedication-mode-tabs button').removeClass('primary secondary').addClass('secondary');
            $el.find(`#ded-mode-${currentMode}`).removeClass('secondary').addClass('primary');
        }

        async function renderDedicationContent() {
            $status.empty();
            if (currentMode === 'none') {
                $content.html('<p style="font-size:0.85rem; color:#999; font-style:italic;">No dedication page will be included.</p>');
                // Clear dedication on server
                try {
                    await api.delete(`/projects/${projectId}/dedication`);
                    console.log('[Dedication] Cleared dedication');
                } catch (err) {
                    console.warn('[Dedication] Failed to clear:', err.message);
                }
        ...
        async function renderTextEditor() {
            ...
            try {
                ...
                console.warn('[Dedication] Could not load text:', err.message);
            }

            $content.html(`
                <div class="dedication-text-editor">
                    <div class="dedication-toolbar" style="display:flex; gap:0.25rem; margin-bottom:0.5rem; padding:0.25rem; background:#f5f0e8; border-radius:4px;">
                    ...
                    <textarea id="dedication-textarea" rows="6" style="width:100%; padding:0.75rem; border:1px solid var(--tan); border-radius:6px; font-family:var(--font-main); font-size:0.95rem; resize:vertical; text-align:center; line-height:1.8;" placeholder="For Grandma,\n\nWith all my love.\n\n— Joe">${escapeHtml(existingText)}</textarea>
                    <p style="font-size:0.75rem; color:#999; margin-top:0.25rem;">Use **bold** and *italic* for formatting. Each line is centered on the page.</p>
                    <button class="btn primary btn-sm" id="save-dedication-text" style="margin-top:0.5rem;">Save Dedication</button>
                </div>
            `);

            // Toolbar button handlers
            $content.find('.btn-toolbar[data-action="bold"]').on('click', function () {
                console.log('[Dedication] Bold button clicked');
                wrapSelection('**', '**');
            });
            $content.find('.btn-toolbar[data-action="italic"]').on('click', function () {
                console.log('[Dedication] Italic button clicked');
                wrapSelection('*', '*');
            });

            // Save handler
            $content.find('#save-dedication-text').on('click', async function () {
                const text = $content.find('#dedication-textarea').val();
                console.log('[Dedication] Saving text, length:', text.length);
                setLoading($(this), true, 'Saving...');
                try {
                    await api.put(`/projects/${projectId}/dedication/text`, { text });
                    $status.html('<span style="color:#008080; font-size:0.85rem;">✓ Dedication saved</span>');
                    console.log('[Dedication] Text saved successfully');
                } catch (err) {
                    console.error('[Dedication] Save failed:', err.message);
        ...
        function wrapSelection(before, after) {
            const textarea = $content.find('#dedication-textarea')[0];
            ...
            textarea.focus();
            console.log('[Dedication] Wrapped selection with', before, after);
        }

        function renderImageUploader() {
            $content.html(`
                <div class="dedication-image-upload">
                    <div id="dedication-image-preview" style="margin-bottom:0.75rem; text-align:center;"></div>
                    <label style="display:inline-block; padding:0.5rem 1rem; background:var(--btn-primary-bg); color:white; border-radius:6px; cursor:pointer; font-size:0.9rem;">
                        📁 Choose Image
                        <input type="file" id="dedication-image-input" accept="image/png,image/jpeg,image/webp" style="display:none;">
                    </label>
            ...
            $content.find('#dedication-image-input').on('change', async function () {
                const file = this.files[0];
                if (!file) return;
                console.log('[Dedication] Image selected:', file.name, file.size, 'bytes');
                ...
                try {
                    ...
                    console.log('[Dedication] Image uploaded successfully');
                    $status.html('<span style="color:#008080; font-size:0.85rem;">✓ Dedication image saved</span>');
                    // Show preview
                    showLocalPreview(file);
                } catch (err) {
                    console.error('[Dedication] Image upload failed:', err.message);
                    $status.html(`<span style="color:var(--error-red); font-size:0.85rem;">Error: ${escapeHtml(err.message)}</span>`);
                }
            });
        }
        ...
        function loadImagePreview() {
            ...
            if (currentMode === 'image') {
                api.get(`/projects/${projectId}/dedication`).then(data => {
                    if (data.has_image) {
                        $content.find('#dedication-image-preview').html(
                            '<p style="font-size:0.85rem; color:#008080;">✓ Dedication image already uploaded. Choose a new file to replace it.</p>'
                        );
                    }
                }).catch(() => {});
            }
        ...
        function showLocalPreview(file) {
            ...
            reader.onload = function (e) {
                $content.find('#dedication-image-preview').html(
                    `<img src="${e.target.result}" style="max-width:200px; max-height:200px; border:1px solid var(--tan); border-radius:4px;" alt="Dedication image preview">`
                );
            };
    ...
    function renderCoverStep($el, subStep) {
        ...
        if (project.cover_pdf_path) {
            $el.html(`<h2>Cover Design</h2><p style="color:#008080; font-weight:600;">✓ Cover PDF already generated.</p><div class="actions"><a href="/projects/${projectId}/cover" class="btn secondary">Edit Cover</a><button class="btn primary" id="continue-print">Continue to Print →</button></div>`);
            $el.find('#continue-print').on('click', () => { currentStep = 8; renderWizard(); });
            return;
        }
