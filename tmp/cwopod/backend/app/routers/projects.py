...
async def get_project(
    ...
    return {
        ...
        "dropcap_enabled": project.dropcap_enabled,
        "dropcap_font": project.dropcap_font,
        "dropcap_lines": project.dropcap_lines,
        "dropcap_color": project.dropcap_color,
        "dropcap_weight": project.dropcap_weight,
        "dropcap_style": project.dropcap_style,
        "dropcap_padding": project.dropcap_padding,
        "has_presentation_inscription": bool(project.dedication_text or project.dedication_image_path),
    }
...
class PrintOptionsUpdate(BaseModel):
    ...
    dropcap_enabled: bool | None = None
    dropcap_font: str | None = None
    dropcap_lines: int | None = None
    dropcap_color: str | None = None
    dropcap_weight: str | None = None
    dropcap_style: str | None = None
    dropcap_padding: float | None = None
...
async def update_print_options(
    ...
    # Typography fields
    if body.body_font is not None:
        project.body_font = body.body_font

    if body.heading_font is not None:
        project.heading_font = body.heading_font

    if body.heading_size is not None:
        project.heading_size = body.heading_size

    # Drop cap fields
    if body.dropcap_enabled is not None:
        project.dropcap_enabled = body.dropcap_enabled

    if body.dropcap_font is not None:
        project.dropcap_font = body.dropcap_font

    if body.dropcap_lines is not None:
        # Clamp to valid range
        project.dropcap_lines = max(2, min(10, body.dropcap_lines))

    if body.dropcap_color is not None:
        project.dropcap_color = body.dropcap_color

    if body.dropcap_weight is not None:
        project.dropcap_weight = body.dropcap_weight

    if body.dropcap_style is not None:
        project.dropcap_style = body.dropcap_style

    if body.dropcap_padding is not None:
        # Clamp to valid range
        project.dropcap_padding = max(0.0, min(20.0, body.dropcap_padding))
...
async def upload_document(
    ...
    # Validate format
    allowed_extensions = {".epub", ".docx", ".txt", ".pdf", ".html", ".htm"}
...
class PresentationInscriptionTextRequest(BaseModel):
    text: str | None = None


@router.get("/{project_id}/dedication")
async def get_presentation_inscription(
    project_id: uuid.UUID,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Get the presentation inscription page content for a project."""
    logger.info("get_presentation_inscription: project_id=%s, user_id=%s", project_id, current_user.id)
    ...
    project = result.scalar_one_or_none()
    if project is None:
        logger.warning("get_presentation_inscription: project not found, project_id=%s", project_id)
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    logger.info("get_presentation_inscription: project_id=%s, mode=%s", project_id, mode)
    return {
        "mode": mode,
...
async def save_presentation_inscription_text(
    project_id: uuid.UUID,
    body: PresentationInscriptionTextRequest,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Save or clear the presentation inscription page text (supports markdown)."""
    logger.info("save_presentation_inscription_text: project_id=%s, user_id=%s, text_length=%s",
    ...
    project = result.scalar_one_or_none()
    if project is None:
        logger.warning("save_presentation_inscription_text: project not found, project_id=%s", project_id)
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    if project.dedication_text:
        if project.dedication_image_path:
            ...
            if old_path.exists():
                old_path.unlink()
                logger.info("save_presentation_inscription_text: removed old presentation inscription image at %s", old_path)
        project.dedication_image_path = None

    await db.flush()
    logger.info("save_presentation_inscription_text: saved for project_id=%s", project_id)
    return {"mode": "text" if project.dedication_text else "none", "text": project.dedication_text}


@router.put("/{project_id}/dedication/image")
async def save_presentation_inscription_image(
    project_id: uuid.UUID,
    file: UploadFile = File(...),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Upload an image for the presentation inscription page (replaces text if set)."""
    logger.info("save_presentation_inscription_image: project_id=%s, user_id=%s, filename=%s",
    ...
    project = result.scalar_one_or_none()
    if project is None:
        logger.warning("save_presentation_inscription_image: project not found, project_id=%s", project_id)
        raise HTTPException(status_code=404, detail="Project not found")

    # Validate image format
    filename = file.filename or "presentation_inscription.png"
    ext = Path(filename).suffix.lower()
    ...
    # Read and validate size (10 MB max for presentation inscription image)
    content = await file.read()
    ...
    # Remove old presentation inscription image if exists
    if project.dedication_image_path:
        old_path = Path(project.dedication_image_path)
        if old_path.exists():
            old_path.unlink()
            logger.info("save_presentation_inscription_image: removed old image at %s", old_path)

    image_path = storage_dir / f"presentation_inscription{ext}"
    image_path.write_bytes(content)
    logger.info("save_presentation_inscription_image: stored image at %s (%d bytes)", image_path, len(content))
    ...
    await db.flush()
    logger.info("save_presentation_inscription_image: saved for project_id=%s", project_id)
    return {"mode": "image", "has_image": True}


@router.delete("/{project_id}/dedication")
async def clear_presentation_inscription(
    project_id: uuid.UUID,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Clear the presentation inscription page entirely (both text and image)."""
    logger.info("clear_presentation_inscription: project_id=%s, user_id=%s", project_id, current_user.id)
    ...
    project = result.scalar_one_or_none()
    if project is None:
        logger.warning("clear_presentation_inscription: project not found, project_id=%s", project_id)
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    if project.dedication_image_path:
        ...
        if old_path.exists():
            old_path.unlink()
            logger.info("clear_presentation_inscription: removed image at %s", old_path)

    project.dedication_text = None
    project.dedication_image_path = None
    await db.flush()
    logger.info("clear_presentation_inscription: cleared for project_id=%s", project_id)
    return {"mode": "none"}
...
async def get_chapters(
    ...
    """Get chapter list with estimated page numbers for navigation.

    Returns chapters detected from the book text with estimated start pages.
    Page numbers are estimated based on character positions and the project's
    total page count (if available from a previous typeset).
    """
    logger.info("get_chapters: project_id=%s, user_id=%s", project_id, current_user.id)
    ...
    project = result.scalar_one_or_none()
    if project is None:
        logger.warning("get_chapters: project not found, project_id=%s", project_id)
        raise HTTPException(status_code=404, detail="Project not found")

    # Get the text file path
    text_path = project.working_text_path or project.original_text_path
    if not text_path:
        logger.info("get_chapters: no text file for project_id=%s, returning empty", project_id)
        return {"chapters": []}

    try:
        raw_text = Path(text_path).read_text(encoding="utf-8")
    except Exception as e:
        logger.error("get_chapters: failed to read text file %s: %s", text_path, e)
        return {"chapters": []}

    if not raw_text.strip():
        logger.info("get_chapters: text file is empty for project_id=%s", project_id)
        return {"chapters": []}
    ...
    if not text.strip():
        logger.info("get_chapters: text conversion produced empty output for project_id=%s", project_id)
        return {"chapters": []}
    ...
    logger.info("get_chapters: detected %d chapters for project_id=%s", len(chapters), project_id)

    if not chapters:
        return {"chapters": []}

    # Calculate estimated page numbers based on character positions
    total_chars = len(text)
    # Use actual page count if available, otherwise estimate ~2000 chars per page
    # (accounting for front matter, we add ~10 pages for title, TOC, etc.)
    front_matter_pages = 10
    if project.page_count and project.page_count > front_matter_pages:
        body_pages = project.page_count - front_matter_pages
        chars_per_page = total_chars / body_pages if body_pages > 0 else 2000
    else:
        chars_per_page = 2000
        body_pages = max(1, total_chars // chars_per_page)

    logger.debug(
        "get_chapters: total_chars=%d, page_count=%s, chars_per_page=%.1f, body_pages=%d",
        total_chars, project.page_count, chars_per_page, body_pages,
    )

    # Build chapter list with estimated page numbers
    chapter_list = []
    # Use stored chapter_page_map from typeset if available (exact page numbers from PDF bookmarks)
    stored_page_map = project.source_metadata.get("chapter_page_map") if project.source_metadata else {}
    if stored_page_map:
        logger.info(
            "get_chapters: using stored chapter_page_map (%d entries) for project_id=%s",
            len(stored_page_map), project_id,
        )
    else:
        logger.info(
            "get_chapters: no stored chapter_page_map, falling back to character-position estimation for project_id=%s",
            project_id,
        )

    for i, ch in enumerate(chapters):
        stored_page = stored_page_map.get(str(ch.number)) if stored_page_map else None
        if stored_page is not None:
            estimated_page = int(stored_page)
            logger.debug(
                "get_chapters: chapter %d using stored page %d", ch.number, estimated_page
            )
        else:
            # Estimate page number from character position if available and non-zero,
            # otherwise fall back to proportional index estimation.
            if ch.start_position > 0 and total_chars > 0:
                estimated_page = front_matter_pages + 1 + int(ch.start_position / chars_per_page)
            else:
                # start_position is 0 (e.g. EPUB nav without positions) — distribute chapters
                # evenly across body pages based on their index.
                estimated_page = front_matter_pages + 1 + int((i / max(len(chapters), 1)) * body_pages)
            # Ensure odd page (chapters start on right-hand pages)
            if estimated_page % 2 == 0:
                estimated_page += 1
            logger.debug(
                "get_chapters: chapter %d estimated page %d (start_position=%d, index=%d/%d)",
                ch.number, estimated_page, ch.start_position, i, len(chapters),
            )

        # Build a clean display name: combine label + title, strip newlines/extra whitespace
        ch_label = (ch.number_label or "").strip().replace("\n", " ")
        ch_title = (ch.title or "").strip().replace("\n", " ")
        if ch_label and ch_title:
            display_name = f"{ch_label} — {ch_title}"
        elif ch_label:
            display_name = ch_label
        elif ch_title:
            display_name = ch_title
        else:
            display_name = f"Chapter {ch.number}"
    ...
    logger.info(
        "get_chapters: returning %d chapters for project_id=%s, first chapter page=%d, last chapter page=%d",
        len(chapter_list), project_id,
        chapter_list[0]["start_page"] if chapter_list else 0,
        chapter_list[-1]["start_page"] if chapter_list else 0,
    )
