...
const COVER_STEPS = [
    { id: 'prompts', label: 'Prompts', number: 7 },
    { id: 'images', label: 'Images', number: 8 },
    { id: 'builder', label: 'Builder', number: 9 },
];
...
const ALL_STEPS = [
    ...
    { id: 'profanity', label: 'Profanity Filter', number: 3, optional: true },
    { id: 'typos', label: 'Typo Check', number: 4, optional: true },
    { id: 'illustrations', label: 'Illustrations', number: 5, optional: true },
    { id: 'typeset', label: 'Typeset', number: 6 },
    { id: 'cover_prompts', label: 'Prompts', number: 7, group: 'Cover' },
    { id: 'cover_images', label: 'Images', number: 8, group: 'Cover' },
    { id: 'cover_builder', label: 'Builder', number: 9, group: 'Cover' },
    { id: 'print', label: 'Print', number: 10 },
];
...
export async function render($container, params) {
    ...
    let titleWidth = 100, authorWidth = 100; // Width percentages for text elements
    ...
    function renderStep1() {
        ...
        $container.html(`
            ...
            <div class="zone-container" style="margin-top:1rem;">
                ...
                <div class="zone-body">
                    ...
                    ` : ''}
                    <div class="step-nav">
                        <button class="btn secondary" id="back-to-typeset">← Back to Typeset</button>
                        <button class="btn primary" id="continue-step2">Continue to Cover Images →</button>
                    </div>
                </div>
            </div>
        `);
        ...
        $container.find('.use-prompt-step1').on('click', function () {
            ...
            setTimeout(() => {
                ...
                $container.find('#custom-prompt-input').trigger('input');
            }, 400);
        });
    ...
    function renderStep2() {
            ...
            <div class="zone-container" style="margin-top:1rem;">
                ...
                <div class="zone-body">
                    ...
                    <div class="step-nav">
                        ...
                        <button class="btn primary" id="to-builder">Continue to Cover Builder →</button>
                    </div>
                </div>
            </div>`;
        $container.html(html);
        ...
        function updateGenerateButtonState() {
            ...
            // Button is enabled when there's any non-whitespace text
            const hasText = promptText.length > 0;
            $container.find('#gen-custom-img').prop('disabled', !hasText);
        }

        // Update button state initially (will enable since IMAGE_PROMPT_PREFIX has text)
        updateGenerateButtonState();

        // Add input event listener to textarea
        $container.find('#custom-prompt-input').on('input', updateGenerateButtonState);
    ...
    function renderStep3() {
            ...
            <div class="zone-container" style="margin-top:1rem;">
                ...
                <div class="zone-body">
                    ...
                    <div class="step-nav">
                        ...
                        <button class="btn primary" id="assemble-cover">Generate Print-Ready Cover PDF →</button>
                    </div>
                </div>
            </div>
        `);
        ...
        $container.find('#restart-step3').on('click', () => {
            ...
            selectedTemplate = 'classic';
            titleFontSize = 48; titleColor = '#FFFFFF'; titleFontFamily = 'serif';
            titleX = 0.5; titleY = 0.15; titleAnchor = 'center'; titleWidth = 100;
            authorFontSize = 24; authorColor = '#FFFFFF'; authorFontFamily = 'serif';
            authorX = 0.5; authorY = 0.85; authorAnchor = 'center'; authorWidth = 100;
            blurb = null;
        ...
        $container.find('#assemble-cover').on('click', async function () {
            const $btn = $(this);
            ...
            const layout = {
                ...
                title: { text: titleText, font_size: titleFontSize, font_family: titleFontFamily, color: titleColor, x: titleX, y: titleY, anchor: titleAnchor, width: titleWidth },
                author: { text: authorText, font_size: authorFontSize, font_family: authorFontFamily, color: authorColor, x: authorX, y: authorY, anchor: authorAnchor, width: authorWidth },
                blurb: blurb || ''
            };
            try {
    ...
    function renderControls() {
        ...
        $ctrl.find('#title-width').on('input', function () { titleWidth = parseInt($(this).val()); $ctrl.find('#title-width-val').text($(this).val() + '%'); updatePreview(); });
        ...
        $ctrl.find('#author-width').on('input', function () { authorWidth = parseInt($(this).val()); $ctrl.find('#author-width-val').text($(this).val() + '%'); updatePreview(); });
        ...
        $ctrl.find('.template-btn').on('click', function () {
            ...
            if (tmpl) {
                ...
                titleWidth = tmpl.title_position?.width ?? 100;
                ...
                authorWidth = tmpl.author_position?.width ?? 100;
    ...
    function updatePreview() {
        ...
        // Safe zone: keep text at least 10% away from edges (1/4 inch bleed + margin)
        const safeZonePercent = 10;
        const safeZoneStyle = `
            <div style="position:absolute; top:${safeZonePercent}%; left:${safeZonePercent}%; right:${safeZonePercent}%; bottom:${safeZonePercent}%; border:2px dashed rgba(255, 165, 0, 0.6); pointer-events:none; z-index:10;"></div>
            <div style="position:absolute; top:${safeZonePercent - 2}%; left:${safeZonePercent - 2}%; background:rgba(255, 165, 0, 0.9); color:white; padding:2px 6px; border-radius:3px; font-size:0.5rem; z-index:11;">SAFE ZONE</div>
        `;
        ...
        $preview.html(`
            ...
            <div class="cover-panel front" style="position:relative;">
                <span style="font-size:0.6rem; color:#8b4513; position:absolute; top:4px; left:4px; z-index:2;">Front</span>
                ${selectedImage ? `<img src="${escapeHtml(selectedImage)}" style="position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; opacity:0.85;">` : ''}
                ${safeZoneStyle}
                <p style="position:absolute; z-index:1; font-weight:700; left:${titleX * 100}%; top:${titleY * 100}%; transform:translate(-50%, -50%); text-align:center; font-size:${Math.min(titleFontSize * 0.35, 22)}px; color:${titleColor}; font-family:${titleFontFamily}; text-shadow:1px 1px 3px rgba(0,0,0,0.8); word-break:break-word; margin:0; max-width:${titleWidth}%;">${escapeHtml(titleText)}</p>
                <p style="position:absolute; z-index:1; left:${authorX * 100}%; top:${authorY * 100}%; transform:translate(-50%, -50%); text-align:center; font-size:${Math.min(authorFontSize * 0.35, 14)}px; color:${authorColor}; font-family:${authorFontFamily}; text-shadow:1px 1px 2px rgba(0,0,0,0.7); margin:0; max-width:${authorWidth}%;">${escapeHtml(authorText)}</p>
            </div>
            <div style="position:absolute; bottom:0.5rem; left:0; right:0; text-align:center; font-size:0.5rem; color:#666;">
                <span style="display:inline-block; border:2px dashed orange; padding:0 4px; margin:0 4px;">Safe Zone</span>
                Keep text inside this border to avoid cutting
            </div>
        `);
    }
