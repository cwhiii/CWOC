...
export async function render($container, params) {
    ...
    try {
        ...
        projectPageCount = data.page_count || null;
        console.log('[Illustrations] Loaded', illustrations.length, 'illustrations, page_count:', projectPageCount);
    } catch (err) {
    ...
    function getPoolImages(pool) {
        console.log('[Illustrations] getPoolImages — pool:', pool, 'total illustrations:', illustrations.length);
        const result = illustrations.filter(img => {
            const matches = img.pool === pool;
            console.log('[Illustrations] getPoolImages — checking:', img.image_uuid, 'pool:', img.pool, 'matches:', matches);
            return matches;
        });
        console.log('[Illustrations] getPoolImages — returning', result.length, 'images for pool:', pool);
        return result;
    }
    ...
    function renderLayout() {
        ...
        availablePoolComponent = new AvailablePool({
            ...
            onImageUploaded: (imageDTO) => {
                ...
                // Re-attach drag sources so the new thumbnail is draggable
                if (dragDropHandler) {
                    console.log('[Illustrations] onImageUploaded — refreshing drag sources for new thumbnail');
                    dragDropHandler.refreshDragSources();
                }
            },
    ...
    function initSpreadPreview() {
        ...
        const estimatedTotalPages = projectPageCount || Math.max(maxPage, 100);
    ...
    function initLocalReflowEngine() {
        ...
        localReflowEngine = new LocalReflowEngine({
            ...
            onBackendRenderComplete: (result) => {
                console.log('[Illustrations] LocalReflow onBackendRenderComplete — authoritative render received, result:', JSON.stringify(result));
                // Invalidate the page image cache for the rendered pages so SpreadPreview
                // fetches the fresh PNGs that render-preview just wrote to disk.
                // Without this, render() just redraws the stale cached images.
                if (spreadPreview && result && result.rendered_pages) {
                    for (const pageNum of result.rendered_pages) {
                        console.log('[Illustrations] onBackendRenderComplete — invalidating cache for page:', pageNum);
                        delete spreadPreview.pageImageCache[pageNum];
                        delete spreadPreview._pageImageLoading[pageNum];
                        // Also clear the rendered spread key so _requestSpreadRender can re-run
                        const spreadKey = String(pageNum % 2 === 1 ? pageNum : pageNum - 1);
                        delete spreadPreview._renderedSpreads[spreadKey];
                    }
                    console.log('[Illustrations] onBackendRenderComplete — cache invalidated, triggering re-render');
                    spreadPreview.render();
                } else if (spreadPreview) {
                    // Fallback: clear all cached pages for the current spread
                    const spreadStart = spreadPreview.getCurrentSpreadStart();
                    console.log('[Illustrations] onBackendRenderComplete — no rendered_pages in result, clearing spread:', spreadStart);
                    delete spreadPreview.pageImageCache[spreadStart];
                    delete spreadPreview.pageImageCache[spreadStart + 1];
                    delete spreadPreview._pageImageLoading[spreadStart];
                    delete spreadPreview._pageImageLoading[spreadStart + 1];
                    const spreadKey = String(spreadStart % 2 === 1 ? spreadStart : spreadStart - 1);
                    delete spreadPreview._renderedSpreads[spreadKey];
                    spreadPreview.render();
                }
            }
    ...
    function initDragDropHandler() {
        ...
        // Re-attach drop handlers to the canvas after a short delay to ensure it's created
        setTimeout(() => {
            if (dragDropHandler) {
                dragDropHandler.refreshSpreadDropTarget();
            }
        }, 100);
    ...
    function handleDragImagePlaced(imageUuid, placement) {
        ...
        const beforeState = { image_uuid: imageUuid, pool: img.pool, placement: img.placement ? { ...img.placement } : null, lock_state: img.lock_state };
        ...
        actionStack.push({
            type: 'place',
            before: beforeState,
            after: { image_uuid: imageUuid, pool: 'in_book', placement: { ...placement }, lock_state: false },
            description: `Place ${img.label} on page ${placement.page_number} (${placement.mode})`
        });
        updateUndoRedoButtons();

        // Trigger local reflow on placement
        if (placement.mode === 'full_page' || placement.mode === 'plate' || placement.mode === 'inline') {
            console.log('[Illustrations] handleDragImagePlaced — triggering local reflow for', placement.mode, 'placement');
            const reflowResult = triggerLocalReflow('place', img);

            // For inline images, update the height with the snapped value from local reflow
            if (placement.mode === 'inline' && reflowResult && reflowResult.snappedHeightLines) {
                const snappedHeight = reflowResult.snappedHeightLines;
                if (snappedHeight !== placement.height) {
                    console.log('[Illustrations] handleDragImagePlaced — inline height snapped from', placement.height, 'to', snappedHeight, 'lines');
                    placement.height = snappedHeight;
                    img.placement.height = snappedHeight;
                    img.height = snappedHeight;
                }
            }
        }

        // Recalculate DPI warning after placement
        recalculateResolutionWarningForImage(img);

        // Update UI
        selectedImageUuid = imageUuid;
        renderAllPools();
        renderToolbar();
        updateSpreadPreview();
        ...
        if (placement.mode === 'full_page' || placement.mode === 'plate') patchPayload.layout = 'margins';
        if (placement.side) patchPayload.side = placement.side;

        console.log('[Illustrations] handleDragImagePlaced — persisting to backend via PATCH, payload:', JSON.stringify(patchPayload));
        api.patch(`/projects/${projectId}/illustrations/${imageUuid}`, patchPayload)
            .then(result => {
                console.log('[Illustrations] handleDragImagePlaced — backend PATCH success:', JSON.stringify(result));
            })
            .catch(err => {
                console.error('[Illustrations] handleDragImagePlaced — backend PATCH FAILED:', err.message, err);
                const $status = $container.find('#illustrations-status');
                showError($status, 'Failed to save illustration placement: ' + err.message);
            });
    ...
    function handleDragImageResized(imageUuid, newHeight) {
        ...
        const mode = (img.placement && img.placement.mode) || img.placement_mode || 'inline';
        if (mode === 'full_page' || mode === 'plate' || mode === 'inline') {
            console.log('[Illustrations] handleDragImageResized — triggering local reflow for', mode, 'resize');
            const reflowResult = triggerLocalReflow('resize', img);
            
            // For inline images, update the height with the snapped value from local reflow
            if (mode === 'inline' && reflowResult && reflowResult.snappedHeightLines) {
                const snappedHeight = reflowResult.snappedHeightLines;
                if (snappedHeight !== newHeight) {
                    console.log('[Illustrations] handleDragImageResized — inline height snapped from', newHeight, 'to', snappedHeight, 'lines');
                    img.placement.height = snappedHeight;
                    img.height = snappedHeight;
                }
            }
    ...
    /**
     * Re-render all pool components and refresh drag sources.
     * Call this instead of calling each pool's render() individually when
     * drag sources need to stay attached after the re-render.
     */
    function renderAllPools() {
        if (sharedPoolComponent) sharedPoolComponent.render();
        if (availablePoolComponent) availablePoolComponent.render();
        if (inBookPool) inBookPool.render();
        // Re-attach drag sources — pool renders replace DOM nodes, losing draggable attrs
        if (dragDropHandler) dragDropHandler.refreshDragSources();
        console.log('[Illustrations] renderAllPools — pools re-rendered and drag sources refreshed');
    }
    ...
    async function refreshIllustrationsFromBackend() {
        console.log('[Illustrations] refreshIllustrationsFromBackend — ENTRY — fetching fresh data from backend');
        console.log('[Illustrations] refreshIllustrationsFromBackend — projectId:', projectId);
        console.log('[Illustrations] refreshIllustrationsFromBackend — current local illustrations count:', illustrations.length);
        
        // Log current local state before refresh
        illustrations.forEach((img, idx) => {
            console.log(`[Illustrations] refreshIllustrationsFromBackend — BEFORE local[${idx}]:`, JSON.stringify({
                image_uuid: img.image_uuid,
                pool: img.pool,
                lock_state: img.lock_state,
                placement_mode: img.placement_mode,
        ...
        try {
            console.log('[Illustrations] refreshIllustrationsFromBackend — calling GET /projects/' + projectId + '/illustrations');
            const data = await api.get(`/projects/${projectId}/illustrations`);
            console.log('[Illustrations] refreshIllustrationsFromBackend — raw response:', JSON.stringify(data));
            ...
            freshIllustrations.forEach((fresh, idx) => {
                console.log(`[Illustrations] refreshIllustrationsFromBackend — BACKEND[${idx}]:`, JSON.stringify({
                    image_uuid: fresh.image_uuid,
                    pool: fresh.pool,
                    lock_state: fresh.lock_state,
                    placement_mode: fresh.placement_mode,
                    page_number: fresh.page_number,
            ...
            for (const fresh of freshIllustrations) {
                ...
                if (existing) {
                    console.log('[Illustrations] refreshIllustrationsFromBackend — MERGING:', fresh.image_uuid);
                    console.log('[Illustrations] refreshIllustrationsFromBackend — existing BEFORE merge:', JSON.stringify({
                        pool: existing.pool,
                        lock_state: existing.lock_state,
                        page_number: existing.page_number,
                        placement: existing.placement
                    }));
                    console.log('[Illustrations] refreshIllustrationsFromBackend — fresh values:', JSON.stringify({
                        pool: fresh.pool,
                        lock_state: fresh.lock_state,
                        page_number: fresh.page_number,
                        placement: fresh.placement
                    ...
                    console.log('[Illustrations] refreshIllustrationsFromBackend — existing AFTER merge:', JSON.stringify({
                        pool: existing.pool,
                        lock_state: existing.lock_state,
                        page_number: existing.page_number,
                        placement: existing.placement
            ...
            // Log final local state after refresh
            console.log('[Illustrations] refreshIllustrationsFromBackend — AFTER merge, local illustrations:');
            illustrations.forEach((img, idx) => {
                console.log(`[Illustrations] refreshIllustrationsFromBackend — AFTER local[${idx}]:`, JSON.stringify({
                    image_uuid: img.image_uuid,
                    pool: img.pool,
                    lock_state: img.lock_state,
                    placement_mode: img.placement_mode,
            ...
            // Invalidate the page image cache from the current spread onward.
            // The full reflow re-typesets the entire book, so every page from
            // the current spread to the end is stale and must be re-fetched.
            if (spreadPreview) {
                const invalidateFrom = spreadPreview.getCurrentSpreadStart();
                console.log('[Illustrations] refreshIllustrationsFromBackend — invalidating page cache from page', invalidateFrom, 'to end of book');
                spreadPreview.invalidatePagesFrom(invalidateFrom);
            }
            ...
            console.log('[Illustrations] refreshIllustrationsFromBackend — EXIT success');
        } catch (err) {
            console.error('[Illustrations] refreshIllustrationsFromBackend — failed to refresh:', err.message, err);
            console.error('[Illustrations] refreshIllustrationsFromBackend — error details:', JSON.stringify(err));
    ...
    function renderToolbar() {
        ...
        $toolbar.html(`
            <div class="toolbar-controls">
                ...
                <button class="btn btn-sm secondary" id="toolbar-remove" ${disabledAttr} title="Return this image to the Available pool and remove it from the page">
                    &#10006; Remove from page
                </button>
                ${upscaleButtonHtml}
            </div>
        ...
        $toolbar.find('#toolbar-height').on('input', function () {
            let newHeight = parseFloat($(this).val());
            if (isNaN(newHeight) || newHeight < 1) return;
            
            // For inline images, snap height to line boundaries
            if (mode === 'inline' && localReflowEngine) {
                const snappedHeight = localReflowEngine.snapToLineHeight(newHeight);
                // Update the input field with snapped value for immediate feedback
                if (snappedHeight !== newHeight) {
                    $(this).val(snappedHeight);
                }
                newHeight = snappedHeight;
                console.log('[Illustrations][Inline] Height input snapped from', $(this).val(), 'to', newHeight, 'lines');
            }
        ...
        $toolbar.find('#toolbar-height').on('change', function () {
            ...
            if (mode === 'inline' && localReflowEngine) {
                const snappedHeight = localReflowEngine.snapToLineHeight(newHeight);
                // Update the input field with snapped value
                if (snappedHeight !== newHeight) {
                    $(this).val(snappedHeight);
                }
                newHeight = snappedHeight;
                console.log('[Illustrations][Inline] Height change snapped to', newHeight, 'lines');
            }
            ...
            actionStack.push({
                ...
                description: `Resize ${img.label} height to ${newHeight}${mode === 'inline' ? ' lines' : '%'}`
            });
            updateUndoRedoButtons();
        ...
        $toolbar.find('#toolbar-mode').on('change', function () {
            ...
            // Convert height when switching between inline and full_page/plate modes
            if (mode === 'inline' && (newMode === 'full_page' || newMode === 'plate')) {
                // Convert from lines to percentage: assume 8 lines ≈ 80% of page height
                const lines = img.placement.height || 8;
                img.placement.height = Math.min(100, Math.max(10, lines * 10)); // Convert lines to percentage
                console.log('[Illustrations][FullPage] Converting inline height from', lines, 'lines to', img.placement.height, '%');
            } else if ((mode === 'full_page' || mode === 'plate') && newMode === 'inline') {
                // Convert from percentage to lines and snap to line boundaries
                const percent = img.placement.height || 100;
                // Convert percentage to approximate lines: 100% ≈ 10 lines
                const rawLines = Math.max(1, Math.round(percent / 10));
                const snappedLines = localReflowEngine ? localReflowEngine.snapToLineHeight(rawLines) : Math.max(1, Math.round(rawLines));
                img.placement.height = snappedLines;
                console.log('[Illustrations][FullPage] Converting full_page height from', percent, '% to', snappedLines, 'lines (snapped from raw:', rawLines, ')');
            }
        ...
        // Remove from page button — returns image to available pool
        $toolbar.find('#toolbar-remove').on('click', function () {
            console.log('[Illustrations] "Remove from page" button clicked for:', img.image_uuid, img.label);
    ...
    async function handleLockAll() {
        ...
        console.log('[Illustrations] handleLockAll() ENTRY — locking all unlocked in_book images');
        console.log('[Illustrations] handleLockAll — projectId:', projectId);
        console.log('[Illustrations] handleLockAll — typeof projectId:', typeof projectId);
        
        // Log the full illustrations array state
        console.log('[Illustrations] handleLockAll — total illustrations count:', illustrations.length);
        console.log('[Illustrations] handleLockAll — FULL ILLUSTRATIONS DUMP:');
        illustrations.forEach((img, idx) => {
            console.log(`[Illustrations] handleLockAll — illustrations[${idx}]:`, JSON.stringify({
                image_uuid: img.image_uuid,
                label: img.label ? img.label.substring(0, 30) : null,
                pool: img.pool,
                lock_state: img.lock_state,
                placement_mode: img.placement_mode,
                page_number: img.page_number,
        ...
        // Count by pool
        const poolCounts = {
            available: illustrations.filter(i => i.pool === 'available').length,
            in_book: illustrations.filter(i => i.pool === 'in_book').length,
            shared: illustrations.filter(i => i.pool === 'shared').length,
            other: illustrations.filter(i => !['available', 'in_book', 'shared'].includes(i.pool)).length
        };
        console.log('[Illustrations] handleLockAll — pool counts:', JSON.stringify(poolCounts));
        
        const inBookImages = getPoolImages('in_book');
        console.log('[Illustrations] handleLockAll — getPoolImages("in_book") returned:', inBookImages.length, 'images');
        console.log('[Illustrations] handleLockAll — in_book images DUMP:');
        inBookImages.forEach((img, idx) => {
            console.log(`[Illustrations] handleLockAll — in_book[${idx}]:`, JSON.stringify({
                image_uuid: img.image_uuid,
                label: img.label ? img.label.substring(0, 30) : null,
                pool: img.pool,
                lock_state: img.lock_state,
        ...
        console.log('[Illustrations] handleLockAll — unlocked in_book count:', unlocked.length);
        console.log('[Illustrations] handleLockAll — unlocked images DUMP:');
        unlocked.forEach((img, idx) => {
            console.log(`[Illustrations] handleLockAll — unlocked[${idx}]:`, JSON.stringify({
                image_uuid: img.image_uuid,
                label: img.label ? img.label.substring(0, 30) : null,
                pool: img.pool,
                lock_state: img.lock_state
        ...
        if (unlocked.length === 0) {
            console.log('[Illustrations] handleLockAll — no unlocked images in LOCAL state, nothing to do');
            console.log('[Illustrations] handleLockAll — *** THIS MEANS EITHER: ***');
            console.log('[Illustrations] handleLockAll — 1. No images have pool="in_book" in local state');
            console.log('[Illustrations] handleLockAll — 2. All in_book images are already locked');
            console.log('[Illustrations] handleLockAll — *** CHECK: Did insertImageIntoBook persist pool change to backend? ***');
            console.log('='.repeat(80));
            return;
        }
        ...
        const $status = $container.find('#illustrations-status');
        try {
            const endpoint = `/projects/${projectId}/illustrations/lock-all`;
            console.log('[Illustrations] handleLockAll — calling POST', endpoint);
            console.log('[Illustrations] handleLockAll — NOTE: Backend will query DB for pool=in_book, lock_state=false');
            console.log('[Illustrations] handleLockAll — If backend returns locked_count=0, it means DB has no in_book images');
            
            const response = await api.post(endpoint);
            
            console.log('[Illustrations] handleLockAll — api.post() returned');
            console.log('[Illustrations] handleLockAll — raw response:', JSON.stringify(response));
            console.log('[Illustrations] handleLockAll — response.task_id:', response.task_id);
            console.log('[Illustrations] handleLockAll — response.taskId:', response.taskId);
            console.log('[Illustrations] handleLockAll — response.locked_count:', response.locked_count);
            
            const taskId = response.task_id || response.taskId;
            const lockedCount = response.locked_count;
            
            console.log('[Illustrations] handleLockAll — extracted taskId:', taskId);
            console.log('[Illustrations] handleLockAll — extracted lockedCount:', lockedCount);
            
            if (lockedCount === 0) {
                console.warn('[Illustrations] handleLockAll — *** BACKEND RETURNED locked_count=0 ***');
                console.warn('[Illustrations] handleLockAll — This means the backend found NO images with pool=in_book AND lock_state=false');
                console.warn('[Illustrations] handleLockAll — But frontend has', unlocked.length, 'unlocked in_book images in LOCAL state');
                console.warn('[Illustrations] handleLockAll — *** ROOT CAUSE: insertImageIntoBook did NOT persist pool change to backend ***');
            }

            // Update local state to match backend
            console.log('[Illustrations] handleLockAll — updating local state for', unlocked.length, 'images');
            for (const img of unlocked) {
                const before = { image_uuid: img.image_uuid, lock_state: img.lock_state };
                console.log('[Illustrations] handleLockAll — setting lock_state=true for:', img.image_uuid);
                img.lock_state = true;
                actionStack.push({
                    type: 'lock_all',
                    before,
            ...
            if (taskId) {
                console.log('[Illustrations] handleLockAll — polling reflow status for task_id:', taskId);
                await pollReflowStatus(taskId);
            } else {
                console.log('[Illustrations] handleLockAll — no task_id returned, assuming immediate completion');
                hideReflowSpinner();
                console.log('[Illustrations] handleLockAll — calling refreshIllustrationsFromBackend()');
                await refreshIllustrationsFromBackend();
            }
            console.log('[Illustrations] handleLockAll() EXIT — success');
            console.log('='.repeat(80));
        } catch (err) {
            console.error('[Illustrations] handleLockAll — api.post() FAILED');
            console.error('[Illustrations] handleLockAll — error name:', err.name);
            console.error('[Illustrations] handleLockAll — error message:', err.message);
            console.error('[Illustrations] handleLockAll — error:', err);
            console.error('[Illustrations] handleLockAll — error JSON:', JSON.stringify(err, Object.getOwnPropertyNames(err)));
            if (err.response) {
                console.error('[Illustrations] handleLockAll — err.response:', err.response);
            }
            if (err.body) {
                console.error('[Illustrations] handleLockAll — err.body:', err.body);
            }
            if (err.data) {
                console.error('[Illustrations] handleLockAll — err.data:', err.data);
            }
            hideReflowSpinner();
     ...
     */
    async function insertImageIntoBook(img, mode, side) {
        console.log('[Illustrations][ContextMenu] insertImageIntoBook — ENTRY uuid:', img.image_uuid, 'mode:', mode, 'side:', side, 'from pool:', img.pool);

        const beforeState = { image_uuid: img.image_uuid, pool: img.pool, placement: img.placement ? { ...img.placement } : null, lock_state: img.lock_state };

        // Move to in_book pool and force unlocked so the Lock button works
        img.pool = 'in_book';
        img.lock_state = false;
        ...
        console.log('[Illustrations][ContextMenu] insertImageIntoBook — currentPage:', currentPage, 'targetPage:', targetPage);

        // For inline images, snap height to line boundaries
        let height;
        if (mode === 'inline') {
            const rawHeight = 8;
            height = localReflowEngine ? localReflowEngine.snapToLineHeight(rawHeight) : Math.max(1, Math.round(rawHeight));
        } else {
            height = 100;
        }
        ...
        console.log('[Illustrations][ContextMenu] insertImageIntoBook — local state set: pool=in_book, lock_state=false, placement=', JSON.stringify(img.placement));

        actionStack.push({
            type: 'insert',
            before: beforeState,
            after: { image_uuid: img.image_uuid, pool: 'in_book', placement: { ...img.placement }, lock_state: false },
        ...
        if (mode === 'full_page' || mode === 'plate' || mode === 'inline') {
            ...
            if (mode === 'inline' && reflowResult && reflowResult.snappedHeightLines) {
                const snappedHeight = reflowResult.snappedHeightLines;
                if (snappedHeight !== img.placement.height) {
                    img.placement.height = snappedHeight;
                    img.height = snappedHeight;
                }
            }
        ...
        // Persist to backend — pool, placement, and lock_state=false
        const patchPayload = {
            pool: 'in_book',
            placement_mode: mode,
            page_number: targetPage,
        ...
        if (mode === 'full_page' || mode === 'plate') patchPayload.layout = 'margins';
        if (mode === 'plate') patchPayload.side = side;

        console.log('[Illustrations][ContextMenu] insertImageIntoBook — persisting to backend via PATCH, payload:', JSON.stringify(patchPayload));
        try {
            const result = await api.patch(`/projects/${projectId}/illustrations/${img.image_uuid}`, patchPayload);
            console.log('[Illustrations][ContextMenu] insertImageIntoBook — backend PATCH success:', JSON.stringify(result));
        } catch (err) {
            console.error('[Illustrations][ContextMenu] insertImageIntoBook — backend PATCH FAILED:', err.message, err);
            const $status = $container.find('#illustrations-status');
            showError($status, 'Failed to save illustration placement: ' + err.message);
        }

        console.log('[Illustrations][ContextMenu] insertImageIntoBook — EXIT');
    }
     ...
     * @param {object} img - The illustration DTO
     */
    async function handleRemoveFromBook(img) {
        ...
        console.log('[Illustrations][ContextMenu] Image returned to available pool locally:', img.image_uuid);
        ...
        // Persist to backend
        console.log('[Illustrations][ContextMenu] handleRemoveFromBook — persisting pool=available to backend');
        try {
            await api.patch(`/projects/${projectId}/illustrations/${img.image_uuid}`, {
                pool: 'available',
                placement_mode: null,
                page_number: null,
                position_x: null,
            ...
            console.log('[Illustrations][ContextMenu] handleRemoveFromBook — backend PATCH success');
        } catch (err) {
            console.error('[Illustrations][ContextMenu] handleRemoveFromBook — backend PATCH FAILED:', err.message, err);
            const $status = $container.find('#illustrations-status');
            showError($status, 'Failed to remove image from book: ' + err.message);
        }
    }
