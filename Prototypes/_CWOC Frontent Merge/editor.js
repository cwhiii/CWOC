...
let timerModal;
let timers = []; // Array to hold timer objects
let editingTimerIndex = null;

// Stopwatch Modals and Data
let stopwatchModal;
let stopwatches = []; // Array to hold stopwatch objects
...
let draggedGroupData = [];
// Store data objects of the dragged item and its children
...
function toggleAllDay(event, initialLoad = false) {
  if (event) event.stopPropagation();
  const allDayHiddenInput = document.getElementById("all_day");
  ...
  const allDayIcon = allDayToggleButton.querySelector("i");

  if (!initialLoad) {
    allDayHiddenInput.value =
      allDayHiddenInput.value === "true" ? "false" : "true";
  }

  const isAllDay = allDayHiddenInput.value === "true";
...
function updateArchivedIcon(isArchived) {
  ...
  const hiddenArchivedInput = document.getElementById("archived");
  if (isArchived) {
    ...
    hiddenArchivedInput.value = "true";
  } else {
    ...
    hiddenArchivedInput.value = "false";
  }
  applyArchivedState();
}
...
function toggleArchived() {
  const hiddenArchivedInput = document.getElementById("archived");
  const isCurrentlyArchived = hiddenArchivedInput.value === "true";
  updateArchivedIcon(!isCurrentlyArchived);
}

function applyArchivedState() {
  const hiddenArchivedInput = document.getElementById("archived");
  const mainEditor = document.getElementById("mainEditor");
  const isArchived = hiddenArchivedInput.value === "true";
  if (isArchived) {
...
function openNotesModal(event) {
  if (event) event.stopPropagation();
  const markdownContent = getMarkdownFromContentEditable(notesMarkdownInput);
...
function closeNotesModal(saveChanges) {
  if (saveChanges) {
    ...
    setMarkdownToContentEditable(notesMarkdownInput, markdownContent);
    updateNotesPreview();
  }
...
function showInputs(event, indicator) {
  ...
  switch (indicator) {
    ...
    //   inputHTML = `<input type="number" step="${config.step}" placeholder="${currentUnit}" id="waistValue" onkeydown="if(event.key === 'Enter') saveHealthData(event, '${indicator}')"> <button type="button" onclick="saveHealthData(event, '${indicator}')">+Add</button>`;
    //   break;
...
function checkNotifications() {
  ...
  const dueDatetimeInput = document.getElementById("due_datetime").value;
  const dueTimeInput = document.getElementById("due_time").value;
...
function saveChit() {
  ...
  console.log({
    ...
    checklist: checklistItemsData.map((item) => ({
      text: item.text,
      checked: item.checked,
      subItem: item.indentLevel > 0,
    })),
    pinned: document.getElementById("pinned").value === "true",
    archived: document.getElementById("archived").value === "true",
    location: document.getElementById("location").value,
    people: peopleItemsData,
    tags: activeTags,
    color: document.getElementById("color").value,
    healthIndicators: healthData,
    projectItems: projectItemsData,
    alarms: alarms,
    timers: timers.map((t) => {
      const { interval, ...rest } = t;
      return rest;
    }), // Save timers without interval ID
    stopwatches: stopwatches.map((s) => {
      const { interval, ...rest } = s;
      return rest;
    }), // Save stopwatches without interval ID
    notifications: notifications, // Save the new notifications array
  });
}

function deleteChit() {
  showCustomConfirm(
    "Are you sure you want to delete this Chit? (This is a demo, no actual deletion will occur)",
    (confirmed) => {
      if (confirmed) {
        console.log("Chit deletion confirmed (demo).");
      } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initial Data Renders / State Setups
  updateCompactWeather(); // Weather
  updateChecklistCount(); // Checklist
  manageGhostsAndSortLists(); //
  // Checklist render
  renderTagGrid(); // Tags
  renderTagTree(); // Tags
  ...
  updatePinnedIcon(document.getElementById("pinned").value === "true"); //
  // Pinned state
  updateArchivedIcon(document.getElementById("archived").value === "true"); // Archived state
  ...
  if (storedTimers) {
    ...
    // Re-initialize intervals for timers that were running
  ...
  const storedStopwatches = localStorage.getItem("stopwatches");
  if (storedStopwatches) {
    stopwatches = JSON.parse(storedStopwatches);
    // Re-initialize intervals for stopwatches that were running
    stopwatches.forEach((stopwatch) => {
      if (stopwatch.running) {
        stopwatch.interval = setInterval(
          () => updateStopwatch(stopwatch.id),
          10,
        );
      }
    });
  }
  // Load notifications from localStorage (NEW)
  const storedNotifications = localStorage.getItem("notifications");
  if (storedNotifications) {
    notifications = JSON.parse(storedNotifications);
  }

  renderAlertItems(); // This now renders all alert types
...
function changeAlertItemType(event, itemId) {
  ...
  if (newType !== oldType) {
    ...
    if (sourceArray === alarms) {
      ...
      localStorage.setItem("alertsItemsData", JSON.stringify(alertsItemsData));
    }
...
function handleAlertItemDrop(e, targetType, targetItemId) {
  ...
  // Persist changes to localStorage for all relevant types
  localStorage.setItem(
    "alarms",
    JSON.stringify(
      alarms.map((a) => {
        const { interval, ...rest } = a;
        return rest;
      }),
    ),
  );
  ...
  localStorage.setItem("notifications", JSON.stringify(notifications)); // NEW
  localStorage.setItem("alertsItemsData", JSON.stringify(alertsItemsData));
...
function triggerNotificationManual(event, id) {
  ...
  if (notification) {
    showNotification("Notification", "Unnamed Notification", false);
  }
}
...
function addTimer() {
  ...
  localStorage.setItem(
    ...
    JSON.stringify(
      timers.map((t) => {
        const { interval, ...rest } = t; // Don't save interval ID
        return rest;
      }),
    ),
  );
...
function addStopwatch() {
  ...
  localStorage.setItem(
    ...
    JSON.stringify(
      stopwatches.map((s) => {
        const { interval, ...rest } = s; // Don't save interval ID
        return rest;
      }),
    ),
  );
...
function selectColor(color) {
  ...
  // Optional: Save the selected color to local storage for persistence
