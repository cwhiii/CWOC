...
CWOC supports explicit habit tracking via an opt-in model. Any chit can become a habit, with its own goal/progress system, charts, reset periods, and a dedicated Habits view in the Tasks view.
...
- **Auto-enable from Habits mode** — Creating a new chit from the Tasks view while in Habits mode auto-enables habit mode on the new chit
...
The chit editor includes a **🎯 Habits** collapsible zone (visible only when habit mode is on). It contains three collapsible sections:
...
When the Tasks view is active, the sidebar shows a View Mode section with three buttons: **📋 Tasks**, **🎯 Habits**, and **📌 Assigned**. Click "Habits" to enter Habits mode. Your selection is remembered across sessions. Only chits explicitly marked as habits appear here — not all recurring chits.
...
- **Show on calendar views** — When off, the habit is hidden from all calendar views but still appears in the Habits view. Default for new habits is controlled by Settings → Habits
...
**See also:** [Views](views.md) · [Chit Editor](editor.md) · [Cron Triggers & Habit Rules](cron-triggers.md) · [Settings](settings.md)
