...
Press `W` on the dashboard to open the full Weather page. Press `Shift+W` for a quick weather modal showing the current forecast for your default saved location, including weather icon, high/low temperatures, and precipitation. Temperatures display in °F or °C based on your unit system setting (Settings → Unit System). If no default location is configured, the modal will prompt you to set one in Settings. The modal includes a **📊 Full Forecast** button that opens the dedicated Weather page.

In the chit editor, weather loads automatically when a chit has both a location and a date. Stored forecast data displays immediately on load, then a live fetch updates it in the background. Placeholder messages appear when either location or date is missing.
...
**See also:** [Saved Locations](saved-locations.md) · [Calendar](calendar.md) · [Chit Editor](editor.md) · [Settings](settings.md)
