...
CWOC includes a built-in email client that supports multiple email accounts. Configure your accounts in Settings → Email → Accounts. Each account has its own IMAP/SMTP server settings, while syncing configuration (max pull, check interval) and signature are shared across all accounts. Emails are stored as chits with special email fields.

## Multiple Accounts

- **Add Account** — Click "Add Account" in Settings → Email to add a new email account. You can add any number of accounts, including third-party providers (Gmail, Outlook, Yahoo, custom IMAP servers, etc.)
...
- **Signature** — Configure a markdown signature in Settings → Email → Signature. It is automatically appended to all outgoing emails, converted to HTML
- **Auto-check** — Configure automatic mail checking intervals in Settings → Email → Syncing (5, 15, 30, or 60 minutes)
...
- **Create Bundle** — Click the "+" button at the end of the tab row. Enter a name and optional description, then click "Define Rule" to set up the classification rule in the Rule Editor
...
- **Multi-Placement** — By default, each email goes to the first matching bundle only. Enable "Allow Multi-Placement" in Settings → Email to let emails appear in multiple bundles simultaneously
...
- **Nest Button** — In the chit editor title row (after the owner chip, before the email button), a nest icon appears on all non-email chits. Click it to open the thread picker
...
- **Configure in Settings** — Go to Settings → Badges to enable/disable individual detectors or entire categories, change the max badges per email, or create custom detectors
...
**See also:** [Views](views.md) · [Omni View](omni-view.md) · [Contacts](contacts.md) · [Keyboard Shortcuts](hotkeys.md)
